...
export const generateTravelPlan = async (profile: UserProfile): Promise<TravelPlan> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          required: ["personalityTitle", "personalityDescription", "vibeScore", "itinerary", "localEtiquette", "soundtrack", "movies"]
        }
      }
    });
    ...
    try {
      ...
      logError("Gemini JSON 파싱", parseError);
      throw new Error("시나리오 데이터를 읽는 중 문제가 발생했습니다.");
    }
  } catch (error: any) {
    logError("Gemini API 호출", error);
    throw error;
  }
};
